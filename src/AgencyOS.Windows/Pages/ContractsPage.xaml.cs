...
/// </para>
/// </remarks>
public sealed partial class ContractsPage : Page, IPaletteCommandTarget, IRecordTarget
{
    ...
    /// is asking for one contract and not for whatever this page was last
    /// filtered to.
    /// </remarks>
    public void Reveal(Guid record)
    {
        ...
        EffectiveBox.IsChecked = false;
        DifferencesBox.IsChecked = false;
    ...
    private void SelectRevealed()
    {
        ...
        if (_list.Contracts.FirstOrDefault(x => x.Id == wanted) is not { } row)
        {
            ListError.Message =
                "That contract is not in the rows loaded here. Search for it by title.";
            ListError.IsOpen = true;
            _reveal = null;
        ...
        ContractList.ScrollIntoView(row);
    }
