...
function Invoke-Contract {
    ...
    try {
        # --no-incremental because the document is deleted above: an incremental
        # build with nothing to rebuild skips the generation target entirely, so a
        # second run from an unchanged tree produced no document at all and the
        # gate failed on its own output. The generated bytes were never the
        # problem; running the gate twice was.
        dotnet build $api --nologo --no-incremental -p:OpenApiGenerateDocumentsOnBuild=true
